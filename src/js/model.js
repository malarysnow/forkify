...
export const updateServings = function (newServings) {
  state.recipe.ingredients.forEach(ing => {
    ...
    //newQt = newQt * newServings / oldServings // 2 * 8 / 4 = 4
  });
...
export const uploadRecipe = async function (newRecipe) {
  try {
    //Transform user form data into proper API format
    const ingredients = Object.entries(newRecipe)
      ...
      .map(ing => {
        ...
        if (ingArr.length !== 3)
          throw new Error(
            `Wrong ingredient format. Please use the correct format. Commas and all:)`
          );
